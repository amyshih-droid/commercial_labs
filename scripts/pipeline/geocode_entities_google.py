...
def run_geocode_google(
    ...
    """Helper entry point for running Google geocoding directly from run_pipeline.py"""
    api_key = os.environ.get(api_key_env)
    if not api_key:
        raise EnvironmentError(
            f"No API key found in environment variable '{api_key_env}'. "
            f"Set it in a .env file at your project root."
        )
    ...
    for count, idx in enumerate(rows_to_process, 1):
        ...
        # Save checkpoint every 50 rows just in case
        if count % 50 == 0 or count == len(rows_to_process):
            output_file.parent.mkdir(parents=True, exist_ok=True)
            df.to_csv(output_file, index=False)
            print(f"  Progress: {count}/{len(rows_to_process)} processed "
                  f"({matched_count} matched so far)... saved checkpoint.")

    # Final save
    output_file.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(output_file, index=False)

    print(f"\nCompleted Geocoding: {api_calls} API calls made, {cache_hits} loaded from cache.")
