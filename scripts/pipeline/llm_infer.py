...
Usage:
    python llm_infer.py \
        ...
        --fields website_url,address_street,contact_name,contact_email, is_gmp_facility \
        --limit 5
"""
...
def build_extraction_prompt(company_name: str, city: str, state: str, zip_code: str, missing_fields: list, pages: dict) -> str:
    ...
    gmp_instructions = ""
    if "is_gmp_facility" in missing_fields:
        gmp_instructions = """
For the `is_gmp_facility` field, return exactly one of:

- "explicit_gmp"
  The website explicitly mentions GMP, cGMP, Good Manufacturing Practice,
  GMP-certified, GMP-compliant, or manufacturing under GMP.

- "no_explicit_gmp"
  The website does not explicitly mention GMP or cGMP.

Do NOT infer GMP from:
- ISO 13485
- FDA registration
- cleanrooms
...
Return an `evidence` field containing the exact supporting quote. If the result is "no_explicit_gmp", you can set evidence to null.
"""

    commercial_instructions = ""
    if "is_commercial" in missing_fields:
        commercial_instructions = """
For the `is_commercial` field, evaluate the text and return true or false:
- true: If this is a fee-for-service contract testing lab, CRO/CDMO, private commercial analytical facility, or private testing company.
- false: If this is a university/academic lab, government facility (e.g., EPA, state health dept), municipal plant (e.g., city water), military lab, or hospital internal lab.
...
async def search_gmp_fallback_async(company_name: str, city: str, state: str, client: AsyncOpenAI, tracker: TokenTracker) -> str:
    cleaned_name = clean_company_name(company_name)
    
    prompt = f"""Search the web to determine if the following medical facility / laboratory explicitly claims GMP compliance.
...
Rules:
- Return "explicit_gmp" if official web search results explicitly mention GMP, cGMP, Good Manufacturing Practice, GMP-certified, or GMP-compliant for this company.
- Return "no_explicit_gmp" if the company exists but there is no explicit mention of GMP.
- Do NOT infer GMP from ISO 13485, FDA registration, cleanrooms, or CDMO services alone.
- Return null if you cannot confidently determine this from the search results.

Return ONLY valid JSON:
{{"is_gmp_facility": "<explicit_gmp, no_explicit_gmp, or null>"}}
"""
    try:
        resp = await client.responses.create(
            model="gpt-5.4-nano",
        ...
        parsed = parse_json_response(resp.output_text)
        if isinstance(parsed, dict):
            return parsed.get("is_gmp_facility")
        return None
    except Exception:
        return None
...
async def process_row_async(batch_idx: int,
    ...
    async with semaphore:
        ...
        # Step 8: GMP FALLBACK - If is_gmp_facility is STILL missing (e.g. scraper blocked)
        current_gmp_val = results.get("is_gmp_facility", {}).get("value")
        if "is_gmp_facility" in missing_fields and current_gmp_val in [None, "", "no_explicit_gmp"]:
            print(f"  Double-checking GMP via Google Search for {company_name}...")
            fallback_gmp = await search_gmp_fallback_async(company_name, city, state, client, tracker)
            # Only overwrite if the search actually found explicit GMP proof
            if fallback_gmp == "explicit_gmp":
                results["is_gmp_facility"] = {"value": fallback_gmp, "status": "found_via_fallback_search"}
        save_to_cache(key, results)
        return global_idx, results


async def run_pipeline_async(df: pd.DataFrame, fields: list, limit: int, output_file: Path = None, checkpoint_interval: int = 10,):
    client = AsyncOpenAI()
    tracker = TokenTracker()
    ...
    async with aiohttp.ClientSession() as session:
        # Process rows in mini-batches/chunks
        for chunk_start in range(0, total_to_process, checkpoint_interval):
            chunk_indices = rows_needing_work[
                chunk_start : chunk_start + checkpoint_interval
            ]

            tasks = []
            for b_offset, g_idx in enumerate(chunk_indices, 1):
                b_idx = chunk_start + b_offset
                row = df.loc[g_idx]
            ...
            # Wait for current mini-batch to finish
            chunk_results = await asyncio.gather(*tasks)

            # 1. Update DataFrame in place for this chunk
            for g_idx, row_results in chunk_results:
                row_missing_fields = [
                    f for f in fields if pd.isna(df.loc[g_idx, f])
                ]
            ...
            # 2. Save incremental progress to disk
            if output_file:
                df.to_csv(output_file, index=False)
                processed_count = min(chunk_start + checkpoint_interval, total_to_process)
                print(f"  [Checkpoint Saved] Updated CSV at {processed_count}/{total_to_process} rows.")
...
def run_llm_infer(input_file: Path, output_file: Path, fields: list = None, limit: int = None, checkpoint_interval: int = 10):
    ...
    # Run the async extraction loop in chunks with periodic checkpoint saves
    asyncio.run(run_pipeline_async(df=df,
            fields=fields,
            limit=limit,
            output_file=output_file,
            checkpoint_interval=checkpoint_interval,))
    
    print(f"\nSuccessfully finished! Final output written to {output_file}")
...
def main():
    ...
    asyncio.run(run_pipeline_async(df, fields, args.limit, output_file=args.out))
