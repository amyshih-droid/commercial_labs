"""
BioPharmGuy CRO / Services Directory Connector

Responsibilities
----------------
- Connect to BioPharmGuy Contract Research & Scientific Services sub-directories via HTTP
- Extract CRO and service provider company listings
...
DEFAULT_CONFIG_PATH = Path("config/connectors/biopharmguy_cros.yaml")
...
def parse_category_table(html: str, category_name: str, base_url: str) -> list[dict]:
    """Parses BioPharmGuy CRO/Services table grids into structured raw records."""
    ...
    for table in tables:
        ...
        for row in rows:
            ...
            invalid_names = ["COMPANY", "LOCATION", "NAME", "WEBSITE", "BACK TO TOP", "TOP"]
            if not name or len(name) < 2 or name in invalid_names:
                continue

            if href and (href.startswith("#") or href == "/"):
                href = ""
            if href and any(nav in href for nav in ["company-by-location", "biotech-company-directory", "biotech-service-provider"]):
                href = ""
...
def validate(df: pd.DataFrame, config: dict) -> None:
    ...
    if df.empty:
        raise ValueError("BioPharmGuy CRO connector returned zero rows.")
    ...
    if not df.columns.is_unique:
        raise ValueError("Duplicate columns detected in BioPharmGuy CRO DataFrame.")
...
def extract(
    *,
    ...
    """Extract BioPharmGuy CRO/Services directory listings driven by YAML config."""
    ...
    for category_name, endpoint in endpoints.items():
        ...
        logger.info("Extracting BioPharmGuy CRO category: %s (%s)", category_name, target_url)
    ...
    validate(df, config)
    logger.info("Successfully extracted %d total BioPharmGuy CRO entries.", len(df))
    return df
