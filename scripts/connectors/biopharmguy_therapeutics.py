"""
BioPharmGuy Directory Connector

Responsibilities
----------------
- Connect to BioPharmGuy sub-directories via HTTP requests (with stealth fallback)
- Extract company listings across target categories
...
DEFAULT_CONFIG_PATH = Path("config/connectors/biopharmguy_therapeutics.yaml")
...
def parse_category_table(html: str, category_name: str, base_url: str) -> list[dict]:
    """Parses BioPharmGuy category table grids into structured raw records."""
...
def validate(df: pd.DataFrame, config: dict) -> None:
    ...
    if df.empty:
        raise ValueError("BioPharmGuy connector returned zero rows.")
    ...
    if not df.columns.is_unique:
        raise ValueError("Duplicate columns detected in BioPharmGuy DataFrame.")
...
def extract(
    *,
    ...
    """Extract BioPharmGuy directory listings driven by YAML config."""
    ...
    for category_name, endpoint in endpoints.items():
        ...
        logger.info("Extracting BioPharmGuy category: %s (%s)", category_name, target_url)
    ...
    validate(df, config)
    logger.info("Successfully extracted %d total BioPharmGuy entries.", len(df))
    return df
